    ...
    def bag_flow_run(self,
        ...
        for bag in range(self.n_bags):
            ...
            if X_valid is not None and y_valid is not None and self.user_split:
                ...
                history = model.fit_generator(
                    ...
                    validation_data=self.valid_datagen.flow(
                        ...
                        shuffle=False),
                    validation_steps=X_valid.shape[0] / self.batch_size,
            ...
            if self.predict_test and X_test is not None:
                self.predictions_test.append(self.predict_test_augment(
                    X_test, model))
        ...
        if self.predict_test and X_test is not None:
            return model, np.array(self.predictions_valid), np.array(self.predictions_valid)
    ...
    def kf_flow_run(self,
        ...
        for train_index, test_index in kf.split(X_train, y_train_split):
            ...
            if self.verbose:
                print('Validation split - standard deviation for original target values: {} \n \
                for predicted target values: {} \n \n'.format(
                    np.std(y_valid), np.std(self.oof_train[test_index, :])))

            if self.predict_test and X_test is not None:
                print('Predicting on test data.')
                self.oof_test[:, :, self.i -
                              1] = self.predict_test_augment(X_test, model)
